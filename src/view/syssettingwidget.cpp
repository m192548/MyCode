...
void SysSettingWidget::loadconfig()
{
    ...
    QString videoSubPath="fileOut/video";
    QString videoCover=videoSubPath+"/cover";
    QString photoPath=
    //加载视频配置
    this->videoPath=setting.value("video_path","").toString();
    if(this->videoPath.isEmpty())
    {
        QDir outPutDir(appPath);
        if(!outPutDir.mkpath("fileOut/video/cover"))
        {
            qDebug()<<"创建视频输出目录失败！";
            return;
        }

        if(!outPutDir.mkpath("fileOut/photo"))
        {
            qDebug()<<"创建图片输出目录失败！";
            return;
        }
